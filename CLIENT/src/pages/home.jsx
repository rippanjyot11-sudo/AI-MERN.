...
// Safe image imports (add real images in /assets, else fallback to placeholder)
import historyAnalyticsImg from '../assets/history_analytics.png'
import resumeImg from '../assets/resume-based-interview.png'
import downloadableImg from '../assets/downloadble-pdf-report.png'
import aiAnswerEvaluationImg from '../assets/ai-answer-evaluation.png'
import hrImg from '../assets/hr.png'
import creditImg from '../assets/creditss.png'
...
function Home() {
  ...
  const navigate = useNavigate()                             // ye js  hai
  const { userData } = useSelector((state) => state.user)

  return (
    <div className='min-h-screen bg-[#f3f3f3] flex flex-col'>   {/* ye jsx hai*/}
      {/* Navbar */}
   {Navbar && <Navbar />}
      ...
      <div className='flex-1 px-6 py-20'>
        ...
        <div className='flex justify-center mb-6'>
          <div className='bg-gray-100 text-gray-600 text-sm px-4 py-2 rounded-full flex items-center gap-2'>
            <HiSparkles size={16} className="bg-green-50 text-green-600 rounded-full" />
       ...
       <div className='mb-32'>
          ...
          <div className='grid md:grid-cols-2 gap-10'>
            {[
              { image: aiAnswerEvaluationImg, icon: <BsBarChart size={20} />, title: "AI Answer Evaluation", desc: "Scores communication, technical accuracy and confidence." },
              ...
              { image: downloadableImg, icon: <BsFileEarmarkText size={20} />, title: "Downloadable PDF Report", desc: "Detailed strengths, weaknesses and improvement insights." },
              { image: historyAnalyticsImg, icon: <BsBarChart size={20} />, title: "History & Analytics", desc: "Track progress with performance graphs and analysis." },
            ].map((item, index) => (
              <motion.div
                key={index}
                initial={{ opacity: 0, y: 30 }}
...
<div className='mb-32'>
          ...
          <div className='grid md:grid-cols-2 gap-10'>
            {[
              { image: hrImg, title: "HR Interview Mode", desc: "Behavioral and communication based Evaluation." },
              { image: techImg, title: "Technical Mode", desc: "Deep technical questioning based on selected role." },
              { image: confidenceImg, title: "Confidence Detection", desc: "Basic tone and voice analysis insights." },
              { image: creditImg, title: "Credits System", desc: "Unlock premium interview sessions easily." },
            ].map((mode, index) => (
              <motion.div
                key={index}
                initial={{ opacity: 0, y: 30 }}
